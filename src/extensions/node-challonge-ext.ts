...
function getMatches(linkData: { subdomain: any; path: any; }, origCallback: ListenForCb | undefined) {
	client.setSubdomain(linkData.subdomain);
	client.tournaments.show({
		id: linkData.path,
		callback: (err: any, tournamentData: { tournament: Record<string, string> }) => {
			if (err) {
			...
			client.matches.index({
				id: linkData.path,
				callback: (err: any, matchData: Record<string, string>[]) => {
					if (err) {
						nodecg.log.error(err);
						return;
					}
					...
					client.participants.index({
						id: linkData.path,
						callback: (err: any, playerData: Record<string, string>[]) => {
							if (err) {
								nodecg.log.error(err);
								return;
							}
...
function parseMatches(matches: Record<string, any>[], players: Record<string, any>[]) {
	const top8 = [];
	const top8matches = [];
	const top8players: { [id: string]: string } = {};
